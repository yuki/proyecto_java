...
public class Gestor {
    ...
    public void CargarDriver() {
        try {
            ...
            j.showMessageDialog(null,
                    "No se ha podido encontrar el driver para hacer la conexion a la Base de Datos\n El programa terminará ahora",
                    "ERROR en driver", j.ERROR_MESSAGE);
            System.exit(0);
        }
    }
    ...
    public void ConexionBD() {
        try {
            ...
            j.showMessageDialog(null, "No se ha podido acceder a la Base de Datos\n El programa terminará ahora.\n" + e,
                    "ERROR en conexion", j.ERROR_MESSAGE);
            System.exit(0);
        }
    }
    ...
    public void consultarBD(String consulta) {
        try {
            ...
            // JOptionPane j = new JOptionPane();
            // j.showMessageDialog(null,"NO se ha podido realizar la consulta","ERROR en
            // consulta",j.ERROR_MESSAGE);
        }
    }
    ...
    public boolean ExisteE(String user, String pass) {
        ...
        try {
            if (this.rs.next())
                ...
                return false;// throw new NOExisteUsuarioException(user);
        } catch (SQLException e) {
            this.cerrarConsulta();
            // no hay usuarios con ese nombre y password, por lo que NO lanzamos la
            // exception
        }
        return false;
    }
    ...
    public EMPRESAS ExisteEmpresa(String user, String pass) throws NOExisteUsuarioException {
        ...
        try {
            this.rs.next();
            EMPRESAS e = new EMPRESAS(rs.getString(1), rs.getString(2), rs.getString(3), rs.getInt(4), rs.getString(5),
    ...
    public void ConsultarUsuario(String emp, String que, String c) throws ExisteUsuarioException {
        ...
        try {
            ...
            this.cerrarConsulta();
            // no hay usuarios con ese nombre por lo que no necesitamos usar la exception
            // que nos lanza
        }
        this.cerrarConsulta();
    }
    ...
    public boolean InsertarCandidato(CANDIDATOS c) {
        String s = new String("INSERT INTO candidatos VALUES ('" + c.getusuario() + "','" + c.getclave() + "','"
                + c.getnombre() + "','" + c.getapellidos() + "','" + c.getdireccion() + "'," + c.gettelefono() + ",'"
                + c.getemail() + "'," + c.getdia() + "," + c.getmes() + "," + c.getanyo() + ",'" + c.getcarnet() + "','"
                + c.getcoche() + "','" + c.getdispviajar() + "','" + c.getestudios() + "','" + c.getotrosconoz() + "','"
                + c.getexperiencia() + "','" + c.getvidalabor() + "');");
    ...
    public boolean actualizarCandidato(CANDIDATOS c) {
        String s = new String("UPDATE candidatos SET c_clave='" + c.getclave() + "',c_name='" + c.getnombre()
                + "',c_apel='" + c.getapellidos() + "',c_direccion='" + c.getdireccion() + "',c_telefono="
                + c.gettelefono() + ",c_email='" + c.getemail() + "',dia=" + c.getdia() + ",mes=" + c.getmes()
                + ",anyo=" + c.getanyo() + ",carnet='" + c.getcarnet() + "',coche='" + c.getcoche() + "',viajar='"
                + c.getdispviajar() + "',estudios='" + c.getestudios() + "',otros='" + c.getotrosconoz()
                + "',experiencia='" + c.getexperiencia() + "',vida_labo='" + c.getvidalabor() + "' WHERE c_user='"
                + c.getusuario() + "'");
        boolean bien = insertarBD(s);
        cerrarInsertar();
        if (bien)
    ...
    public boolean InsertarEmpresa(EMPRESAS emp) {
        String s = new String("INSERT INTO empresa VALUES ('" + emp.getusuario() + "','" + emp.getclave() + "','"
                + emp.getnombre() + "'," + emp.getcif() + ",'" + emp.getdir() + "'," + emp.gettelefono() + ",'"
                + emp.getemail() + "','" + emp.getdesc() + "','" + emp.getcontacto() + "');");
        boolean bien = insertarBD(s);
        cerrarInsertar();
        if (bien)
            return true;
    ...
    public void InsertarConocimiento(CONOCIMIENTO con) {
        String s = new String(
                "INSERT INTO conocimientos VALUES (" + con.getCODCONOC() + ",'" + con.getNOMBRE() + "');");
        boolean bien = insertarBD(s);
        cerrarInsertar();
    }

    public Object[][] misConocimientos(String user, String como) {
        String s = new String("SELECT c.c_cod, c.c_nombre FROM conocimientos c, cand_conoc ca WHERE ca.c_user= '" + user
                + "' AND c.c_cod" + como + "ca.c_cod ;");
        consultarBD(s);
        Object[][] can;
        try {
            int i = 0;
    ...
    public void InsertarOferta(OFERTA o) {
        String s = new String("INSERT INTO ofertas VALUES (" + o.getCODOFERTA() + ",'" + o.getTITULO() + "','"
                + o.getDESCRIP() + "','" + o.getLUGAR() + "','" + o.getCONTRATO() + "'," + o.getSUELDOMIN() + ","
                + o.getSUELDOMAX() + ",'" + o.getEXPERIENCIA() + "','" + o.getSEVALORARA() + "','"
                + o.getIMPRESCINDIBLE() + "'," + o.getDIA() + "," + o.getMES() + "," + o.getANYO() + ",'" + o.isESTADO()
                + "','" + o.getUSUARIOEMP() + "');");
    ...
    public void ActualizarOferta(String user, int oferta, boolean estado) {
        String t = new String("UPDATE ofertas SET estado = '" + estado + "' WHERE e_user = '" + user + "' AND o_cod = "
                + oferta + ";");
    ...
    public Object[][] BusquedaOfertas(String titulo, String lugar, String contrato, String empresa, int sueldo, int exp,
            String conoc) {
        Object[][] v;// = new Vector();
        ...
        if (conoc != "") {
            ...
            s = s + " o_cod IN (SELECT o_cod FROM oferta_conoc WHERE c_cod = (SELECT c_cod FROM conocimientos WHERE c_nombre = '"
                    + conoc + "'))";
        }
        if (pasado) {
            s = s + " ORDER BY o_cod ;";
    ...
    public void insertarSolicitud(SOLICITUD s) {
        String consulta = new String("INSERT INTO solicitudes VALUES (" + s.getcodigo() + "," + s.getcoferta() + ",'"
                + s.getuser() + "'," + s.getdia() + "," + s.getmes() + "," + s.getanyo() + ");");
    ...
    public Object[][] misSolicitudes(String user) { // si lo llama un candidato
        String s = new String(
                "SELECT  o.estado, s.s_cod, o.titulo, s.o_cod, o.e_user, s.dia, s.mes, s.anyo FROM solicitudes s, ofertas o WHERE s.c_user = '"
                        + user + "' AND o.o_cod=s.o_cod  ORDER BY s.s_cod;");
        consultarBD(s);
        Object[][] v;
        try {
            int i = 0;
    ...
    public Object[][] ConocimientosDeEmpresa(int coferta) {
        ...
        try {
            String s = new String(
                    "SELECT c.c_cod, c.c_nombre FROM conocimientos c, oferta_conoc of WHERE c.c_cod = of.c_cod AND of.o_cod ="
                            + coferta + " ORDER BY c.c_cod");
            consultarBD(s);
            int i = 0;
            while (rs.next())
                i++;
    ...
    public Object[][] SacarSolicitud(String e_user) {// ANALISIS DE SOLICITUDES
        /*
        ...
        try {
            consultarBD(
                    "SELECT o_cod, COUNT (s_cod) FROM empresa e, ofertas o, solicitudes s WHERE e.e_user = o.e_user AND o.o_cod = s.o_cod ORDER BY s_cod;");
            int i = 0;
            while (this.rs.next())
                i++;
            v = new Object[i][2];
    ...
    public Object[][] OfertasASolicitudes(int o_cod) {
        ...
        try {
            String s = new String(
                    "SELECT s_cod FROM solicitudes WHERE o_cod = (SELECT o_cod FROM ofertas WHERE o_cod = " + o_cod
                            + ");");
            consultarBD(s);
            int i = 0;
            while (this.rs.next())
